...
namespace TerrainComposer2.NodePainter
{
	public static class GlobalPainting 
	{
		...
		/// <summary>
		///  Sets up the specified material with painter and visualization state
		/// </summary>
		public static void MatSetupViz(Painting painter, Material mat)
		{
			if (mat == null)
				return;

			// Painter State
			painter.MatSetupBase(mat);
			painter.MatSetupBrush(mat);

			// Viz State
			mat.SetColor("_VizColor", painter.state_InvertBrush ? invertedBrushColor : normalBrushColor);
			mat.SetFloat("_VizOutlineRadius", 0.48f);
			mat.SetFloat("_VizOutlineThickness", 0.002f);
			mat.SetFloat("_VizStrength", 1f);
			mat.SetFloat("_VizStrengthCurvature", 1f);
		}
		...
		public static void ShowTerrainBrush (Painting painter, Vector3 brushPos, float brushSize, float rotation = 0, float painterWorldAspect = 1)
		{ // Show brush at position
			...
			if (BrushProjection != null && BrushProjection.material != null)
			{
				...
				BrushProjection.transform.rotation = Quaternion.Euler (BrushProjection.transform.rotation.eulerAngles.x, rotation, BrushProjection.transform.rotation.eulerAngles.z);
				BrushProjection.orthographicSize = brushSize / 2 / (painter.canvas_Aspect / painterWorldAspect);
				BrushProjection.aspectRatio = painter.canvas_Aspect / painterWorldAspect;

				MatSetupViz(painter, BrushProjection.material);

				BrushProjection.material.SetFloat("_aspect", 1);
		...
		{ // Show canvas projection
			...
			if (CanvasProjection != null && CanvasProjection.material != null)
			{
				...
				if (canvasTex != null)
					CanvasProjection.material.SetTexture ("_projCanvasTex", canvasTex);
				CanvasProjection.material.SetColor ("_projTintCol", (isValue? canvasTerrainVizColor : new Color (1,1,1,0.5f)));
				CanvasProjection.material.SetFloat ("_projStrength", Settings.terrainCanvasOverlayStrength);
