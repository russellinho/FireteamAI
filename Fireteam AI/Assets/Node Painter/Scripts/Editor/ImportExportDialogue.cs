...
//			dialogue.isRAW = true;
		...
		private void Init () 
		{
			...
			{
				format = Painter.canvas_Format;
				textureCount = Painter.canvasTextureCount;
				channelCount = Painter.canvasChannelCount;
				sizeX = Painter.canvas_SizeX;
				sizeY = Painter.canvas_SizeY;
		...
		private void OnGUI () 
		{
			...
			else
			{
				...
				{
					path = EditorUtility.SaveFilePanel ("Export raw File", Painter.canvas_Name, "raw", "Choose a path to save the canvas image to.");
					if (!string.IsNullOrEmpty (path))
					{
