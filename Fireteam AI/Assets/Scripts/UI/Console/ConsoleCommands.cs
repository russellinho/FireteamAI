    ...
    public static partial class ConsoleProcessor
    {
        ...
        private static string GenerateCommandManual(string commandName)
        {
            ...
            else
            {
                ...
                manual += "\n\nDeclared in";
                if (declaringTypes.Count == 1) { manual += $" {declaringTypes[0]}"; }
                else
                {
                    manual += ":";
        ...
        {
            return GetAllCommands().DistinctBy(x => x.CommandName)
        ...
        {
            return GetUniqueCommands().Where(x => !x.MethodData.DeclaringType.Assembly.FullName.StartsWith("QFSW.QC"))
