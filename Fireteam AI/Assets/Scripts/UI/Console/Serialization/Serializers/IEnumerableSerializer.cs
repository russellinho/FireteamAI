...
namespace Koobando.UI.Console.Serializers
{
    ...
    public abstract class IEnumerableSerializer<T> : PolymorphicSerializer<T> where T : class, IEnumerable
    {
        private readonly Pool<StringBuilder> _builderPool = new Pool<StringBuilder>();

        public override string SerializeFormatted(T value, Theme theme)
        {
