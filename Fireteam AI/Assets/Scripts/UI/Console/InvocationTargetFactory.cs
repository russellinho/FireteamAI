using Koobando.UI.Console.Comparators;
    ...
    public static class InvocationTargetFactory
    {
        ...
        public static IEnumerable<object> FindTargets(Type classType, MonoTargetType method)
        {
            switch (method)
            {
                ...
                {
                    return Registry.GetRegistryContents(classType);
                }
        ...
        {
            object target;
            if (Registry.GetRegistrySize(classType) > 0)
            {
                target = Registry.GetRegistryContents(classType).First();
            }
            else
            {
                target = CreateCommandSingletonInstance(classType);
                Registry.RegisterObject(classType, target);
            }
