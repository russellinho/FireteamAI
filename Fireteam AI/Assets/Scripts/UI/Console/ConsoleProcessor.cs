...
using Koobando.UI.Console.Internal;
...
namespace Koobando.UI.Console
{
    ...
    /// <summary>The core processor for everything relating to QC parsing, command invocation, table generation, registries and more.</summary>
    public static partial class ConsoleProcessor
    {
        ...
        private static readonly KoobandoParser _parser = new KoobandoParser();
        private static readonly KoobandoPreprocessor _preprocessor = new KoobandoPreprocessor();
        ...
        {
            if (assembly.HasAttribute<IgnoreAttribute>(false))
            {
                return false;
            }
        ...
        private static void LoadCommandsFromAssembly(Assembly assembly)
        {
            ...
            foreach (Type type in loadedTypes)
            {
                ...
                catch (BadImageFormatException)
                {
                    ...
                    /*
                    if (loggingLevel >= LoggingLevel.Errors)
                    {
        ...
        {
            if (!type.HasAttribute<IgnoreAttribute>(false))
            {
        ...
        public static bool TryAddCommand(CommandData command)
        {
            ...
            {
                string keyName = $"{command.CommandName}({command.ParamCount})";
                bool alreadyExists = !_commandTable.TryAdd(keyName, command);
                ...
                if (alreadyExists)
                {
                    if (loggingLevel >= LoggingLevel.Warnings)
                    {
                        ...
                        Debug.LogWarning($"Quantum Processor Warning: Could not add {fullMethodName} to the table as another method with the same alias and parameter count, {keyName}, already exists.");
                    }
        ...
        public static object InvokeCommand(string commandString)
        {
            ...
            if (command.IsGeneric)
            {
                ...
                if (genericArgNames.Length == expectedArgCount)
                {
                    ...
                    {
                        genericTypes[i] = KoobandoParser.ParseType(genericArgNames[i]);
                    }
                }
                else
                {
